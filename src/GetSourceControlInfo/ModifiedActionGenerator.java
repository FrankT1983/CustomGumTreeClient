...
public class ModifiedActionGenerator
{
    public class ModifiedUpdate extends Update
    {
        protected ITree toNode;

        public ModifiedUpdate(ITree fromNode, ITree toNode, String value)
        {
            super(fromNode,value);
            this.toNode = toNode;
        }

        public ITree getToNode()
        {
            return  this.toNode;
        }
    }
     ...
     */
        ...
        private TIntObjectMap<ITree> origDestTrees;

        private TIntObjectMap<ITree> cpySrcTrees;

        public ModifiedActionGenerator(ITree src, ITree dst, MappingStore mappings) {
            this.origSrc = src;
            ...
            origDestTrees = new TIntObjectHashMap<>();
            for (ITree t: origDst.getTrees())
                origDestTrees.put(t.getId(), t);
                ...
                if (!newMappings.hasDst(x)) {
                    ...
                    if (!x.equals(origDst)) { // TODO => x != origDst // Case of the root
                        ...
                        if (!w.getLabel().equals(x.getLabel())) {
                            actions.add(new ModifiedUpdate(origSrcTrees.get(w.getId()), origDestTrees.get(x.getId()) , x.getLabel()));
